...
def configure():
    ...
    db_name = input(
        "Database name [lastfrontier]: "
    ).strip() or "lastfrontier"

    db_user = input(
        "Database user [lastfrontier]: "
    ).strip() or "lastfrontier"
...
def load_config():
    ...
    return Config(
        open_weather_api_key=os.environ[
            "OPEN_WEATHER_API_KEY"
        ],
        census_api=os.environ[
            "CENSUS_API"
        ],
        db_host=os.environ[
            "DB_HOST"
